...
const HomeScreen: React.FC = () => {
  ...
  const handleCategorySelect = (categoryId: number) => {
    if (state.bluetoothStatus !== 'connected') {
      dispatch({ type: 'SET_ERROR', payload: 'ESP32 belum terhubung. Silakan hubungkan terlebih dahulu.' });
      return;
    }
  ...
  const getBluetoothStatusText = () => {
    switch (state.bluetoothStatus) {
      case 'connected':
        return '🟢 ESP32 Terhubung';
      case 'connecting':
        return '🟡 Menghubungkan ESP32...';
      case 'unavailable':
        return '🔴 Bluetooth Tidak Tersedia';
      default:
        return '🔴 ESP32 Terputus';
    }
  };
  ...
  return (
    <div className="min-h-screen flex flex-col relative overflow-hidden">
      ...
      {/* Bluetooth Connection Status - Fixed at bottom */}
      <div className="relative z-10 p-4 bg-gradient-to-t from-gray-900/90 to-transparent backdrop-blur-sm">
        <div className="w-full max-w-2xl mx-auto">
          ...
          {/* Error Message - Only show when there's an error AND Bluetooth is not connected */}
          {state.error && state.bluetoothStatus !== 'connected' && (
          ...
          <div className="flex flex-col items-center space-y-2">
            ...
            >
              <RefreshCw className={`mr-2 ${state.bluetoothStatus === 'connecting' ? 'animate-spin' : ''}`} size={20} />
              {state.bluetoothStatus === 'connecting' ? 'Menghubungkan...' : 'Hubungkan ESP32'}
            </button>
            
            <p className="text-center text-xs text-gray-400">
              {state.bluetoothStatus === 'unavailable' 
                ? 'Bluetooth tidak tersedia di platform ini'
