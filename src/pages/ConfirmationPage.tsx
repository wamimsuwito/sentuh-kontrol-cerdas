...
const ConfirmationPage = ({ 
  ...
  const { clearTimer } = useAutoReturn({
    onReturn: () => {
      ...
      // Stop limit switch audio when auto-returning
  ...
  const handleBack = () => {
    console.log('🔇 Stopping limit switch audio - back button pressed');
    stopAudio('limit-switch-active');
    clearTimer();
    if (onCancelCountdown) {
      onCancelCountdown();
  ...
  const handleConfirm = () => {
    console.log('🔇 Stopping limit switch audio - confirm button pressed');
    stopAudio('limit-switch-active');
    clearTimer();
    onConfirm();
  };
